...
DOCUMENTATION = """
---
module: thola_check_memory_usage_facts
author: "Thola team"
version_added: "1.0.0"
short_description: "Checks the memory usage for a given device"
description:
    - "Checks the memory usage for a given device with SNMP"
requirements:
...
changed:
    ...
    sample: True
thola_check_memory_usage_facts:
    description: "Memory usage facts"
    returned: always
    type: dict
"""
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            critical_max=dict(type="int", required=False),
            critical_min=dict(type="int", required=False),
            warning_max=dict(type="int", required=False),
            warning_min=dict(type="int", required=False)
        ),
    ...
    if module.params["critical_max"] is None:
        ...
        critical_max = module.params["critical_max"]
    if module.params["critical_min"] is None:
        critical_min = None
    else:
        critical_min = module.params["critical_min"]
    if module.params["warning_max"] is None:
        warning_max = None
    else:
        warning_max = module.params["warning_max"]
    if module.params["warning_min"] is None:
        warning_min = None
    else:
        warning_min = module.params["warning_min"]

    body = thola_client.CheckMemoryUsageRequest(
        memory_usage_thresholds=thola_client.models.Thresholds(
            critical_max=critical_max,
            critical_min=critical_min,
            warning_max=warning_max,
            warning_min=warning_min
    ...
    result = check_api.check_memory_usage(body=body).__str__().replace("\'", "\"").replace("None", "null")
    try:
