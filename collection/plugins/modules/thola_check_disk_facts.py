...
DOCUMENTATION = """
---
module: thola_check_disk_facts
author: "Thola team"
version_added: "1.0.0"
short_description: "Checks the free disk space for a given device"
description:
    - "Checks the free disk space for a given device with SNMP"
requirements:
...
def main():
    ...
    body = thola_client.CheckDiskRequest(
        disk_thresholds=thola_client.models.Thresholds(
            critical_max=critical_max,
            critical_min=critical_min,
            warning_max=warning_max,
            warning_min=warning_min
    ...
    result = check_api.check_disk(body=body).__str__().replace("\'", "\"").replace("None", "null")
    try:
